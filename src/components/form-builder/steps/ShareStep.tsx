...
export const ShareStep: React.FC<ShareStepProps> = ({ step, onPublish }) => {
  ...
  }, [form, state.autoSave.hasUnsavedChanges]);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (onPublish) {
        console.log("📞 Calling onPublish prop");
        success = await onPublish();
      } else {
        console.log("📞 Calling publishForm from context");
        success = await publishForm();
      }

      console.log("📊 Publish result:", success);

      if (success) {
        console.log("✅ Publish successful, waiting for state update...");
        // Small delay to allow state to update
        setTimeout(() => {
          console.log("🔄 Post-publish state check:", {
            published: form?.published,
            publishedAt: form?.publishedAt,
          });
        }, 100);
  ...
  const debugInfo = (
    <div className="fixed top-4 right-4 bg-white p-4 border rounded shadow-lg z-50 text-xs max-w-xs">
      <h4 className="font-bold mb-2">ShareStep Debug</h4>
      <div>Form ID: {form?.id}</div>
      <div>Published: {String(form?.published)}</div>
      <div>isPublished: {String(isPublished)}</div>
      <div>Fields: {form?.fields?.length || 0}</div>
      <div>Unsaved: {String(hasUnsavedChanges)}</div>
      <div>Publishing: {String(isPublishing)}</div>
    </div>
  );
